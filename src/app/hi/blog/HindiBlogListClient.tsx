...
const CATEGORY_COLORS: Record<string, string> = {
  "प्रवेश और परीक्षा तैयारी": "bg-purple-600",
  "नवोदय प्रवेश": "bg-purple-600",
  "विद्यालय समाचार": "bg-teal-600",
  "शैक्षिक योजना": "bg-amber-500",
  "छात्र जीवन": "bg-pink-600",
  "अध्ययन टिप्स": "bg-orange-500",
  "सैनिक स्कूल": "bg-blue-600",
  "शिक्षक संसाधन": "bg-indigo-600",
  "तुलना": "bg-red-600",
  "बोर्ड परीक्षा": "bg-cyan-600",
  "शिक्षक भर्ती": "bg-emerald-600",
  "इंजीनियरिंग प्रवेश": "bg-violet-600",
  "पढ़ाई के टिप्स": "bg-orange-600",
};

const CATEGORY_LIGHT: Record<string, string> = {
  "प्रवेश और परीक्षा तैयारी": "bg-purple-50 text-purple-700 border-purple-200",
  "नवोदय प्रवेश": "bg-purple-50 text-purple-700 border-purple-200",
  "विद्यालय समाचार": "bg-teal-50 text-teal-700 border-teal-200",
  "शैक्षिक योजना": "bg-amber-50 text-amber-700 border-amber-200",
  "छात्र जीवन": "bg-pink-50 text-pink-700 border-pink-200",
  "अध्ययन टिप्स": "bg-orange-50 text-orange-700 border-orange-200",
  "सैनिक स्कूल": "bg-blue-50 text-blue-700 border-blue-200",
  "शिक्षक संसाधन": "bg-indigo-50 text-indigo-700 border-indigo-200",
  "तुलना": "bg-red-50 text-red-700 border-red-200",
  "बोर्ड परीक्षा": "bg-cyan-50 text-cyan-700 border-cyan-200",
  "शिक्षक भर्ती": "bg-emerald-50 text-emerald-700 border-emerald-200",
  "इंजीनियरिंग प्रवेश": "bg-violet-50 text-violet-700 border-violet-200",
  "पढ़ाई के टिप्स": "bg-orange-50 text-orange-700 border-orange-200",
};
  ...
  return (
    <div className="bg-[#035c00] text-white flex items-stretch overflow-hidden rounded-xl mb-6 shadow-sm">
      <div className="flex items-center gap-2 bg-red-600 px-4 py-2.5 flex-shrink-0 text-xs font-extrabold uppercase tracking-widest">
        ...
        <span className="hidden sm:inline">नवीनतम</span>
      </div>
...
function HeroFeatured({ articles }: { articles: HindiArticle[] }) {
  const [main, ...rest] = articles.slice(0, 3);
  if (!main) return null;
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-8">
      <Link href={`/hi/blog/${main.slug}/`}
        ...
        {rest.map((a) => (
          <Link key={a.slug} href={`/hi/blog/${a.slug}/`}
            ...
            <div className="relative p-4 text-white">
              ...
              <span className="text-[10px] text-white/60 mt-1 block">{a.date} · {a.readTime}</span>
            </div>
          </Link>
        ))}
      </div>
...
function ArticleCard({ article }: { article: HindiArticle }) {
  return (
    <motion.article layout initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <Link href={`/hi/blog/${article.slug}/`} className="relative h-44 w-full overflow-hidden flex-shrink-0 block">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <Link href={`/hi/blog/${article.slug}/`}>
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
          ...
          <Link href={`/hi/blog/${article.slug}/`}
            ...
            पढ़ें <FaArrowRight className="text-[9px] group-hover:translate-x-0.5 transition-transform" />
          </Link>
        </div>
      </div>
    </motion.article>
...
function Sidebar({ trending }: { trending: HindiArticle[] }) {
  return (
    <aside className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="bg-[#035c00] px-5 py-3 flex items-center gap-2">
          <FaFire className="text-yellow-400" />
          <h3 className="text-sm font-extrabold text-white uppercase tracking-wider">लोकप्रिय</h3>
        </div>
        <ul className="divide-y divide-gray-50">
          {trending.map((a, i) => (
            <li key={a.slug}>
              <Link href={`/hi/blog/${a.slug}/`} className="flex gap-3 p-4 hover:bg-gray-50 transition-colors group">
      ...
      <div className="bg-gradient-to-br from-[#035c00] to-[#024700] rounded-xl p-5 text-white shadow-lg relative overflow-hidden">
        ...
        <h3 className="font-extrabold text-base mb-1">प्रवेश परीक्षा की तैयारी करें</h3>
        <p className="text-xs text-white/80 leading-relaxed mb-4">
          AMU, JMI, सैनिक स्कूल, मिलिट्री स्कूल और विद्या ज्ञान — साप्ताहिक OMR टेस्ट के साथ विशेष कोचिंग बैच।
        </p>
        <Link href="/coaching"
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <h4 className="text-sm font-bold text-gray-800 mb-2 flex items-center gap-2">
          <FaUser className="text-[#035c00]" /> प्रधानाचार्य से पूछें
        </h4>
        <p className="text-xs text-gray-500 leading-relaxed mb-4">
          प्रवेश और कोचिंग प्रश्नों के लिए प्रधानाचार्य रिफौल हसन से सीधे WhatsApp पर बात करें।
        </p>
        <a href="https://wa.me/919997161490?text=Hello%20Principal%2C%20I%20have%20an%20inquiry."
          ...
          <FaWhatsapp className="text-base" /> WhatsApp पर चैट करें
        </a>
      </div>

      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <h4 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2">
          <FaTag className="text-[#035c00]" /> श्रेणियां ब्राउज़ करें
        </h4>
        <div className="flex flex-wrap gap-2">
          {Object.keys(CATEGORY_LIGHT).map(cat => (
            <Link key={cat} href={`/hi/blog?category=${encodeURIComponent(cat)}`}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
        <h4 className="text-sm font-bold text-amber-900 mb-2 flex items-center gap-2">
          <span></span> क्या आप जानते हैं?
        </h4>
        <p className="text-[11px] text-amber-800 leading-relaxed">
          हमारे AMU और सैनिक स्कूल बैच गणित और उर्दू भाषा के लिए विषय-विशिष्ट शॉर्टकट्स को कवर करते हैं।
        </p>
        <Link href="/faq"
          className="text-xs font-bold text-amber-700 hover:text-amber-900 mt-3 flex items-center gap-1 group">
          FAQ देखें <FaArrowRight className="text-[9px] group-hover:translate-x-1 transition-transform" />
        </Link>
      </div>
    </aside>
  );
...
export default function HindiBlogListClient() {
  ...
  const pageArticles = listArticles.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between mb-8 bg-white px-5 py-4 rounded-xl border border-gray-100 shadow-sm">
        ...
        <div className="relative w-full md:w-72 flex-shrink-0">
          ...
          <input type="text" placeholder="लेख, लेखक खोजें..."
        ...
        <div className="lg:col-span-2">
          {!isDefault && searchQuery && (
            <p className="text-xs text-gray-500 mb-4">
              "<span className="text-[#035c00]">{searchQuery}</span>" के लिए <span className="font-bold text-gray-800">{filtered.length}</span> परिणाम
            </p>
          )}
          <AnimatePresence mode="popLayout">
            {pageArticles.length > 0 ? (
              <motion.div key={`${selectedCategory}-${currentPage}`} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <h3 className="font-bold text-gray-700 mb-1">कोई लेख नहीं मिला</h3>
                <p className="text-xs text-gray-400">कोई अन्य श्रेणी या खोज शब्द आज़माएं।</p>
              </motion.div>
            )}
          </AnimatePresence>
          {totalPages > 1 && (
