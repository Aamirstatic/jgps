...
function ReadingProgressBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-1 bg-gray-200">
      <div
        className="h-full bg-[#035c00] transition-all duration-100"
...
function StickyShareBar({ title }: { title: string }) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 flex flex-col gap-2 items-end">
      ...
      >
        <FaShare className="text-xs" /> Copy Link
      </button>
    </div>
  );
}
        ...
        {faqs.map((faq, i) => (
          ...
          >
            ...
            >
              ...
              {open === i ? (
                <FaChevronUp className="text-[#035c00] flex-shrink-0 mt-0.5 text-sm" />
              ) : (
...
function RelatedArticlesSidebar({
  ...
  const related = articles
    ...
    .sort((a, b) => {
      const aCat = a.category === category ? 1 : 0;
      const bCat = b.category === category ? 1 : 0;
      return bCat - aCat;
    })
    .slice(0, 4);
        ...
        {related.map((a) => (
          ...
          >
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-[#035c00] uppercase tracking-wide mb-1 truncate">
                {a.category}
              </p>
              <h4 className="text-xs font-bold text-gray-800 leading-snug line-clamp-2 group-hover:text-[#035c00] transition-colors">
                {a.title}
              </h4>
    ...
    <div className="my-10 bg-gradient-to-br from-[#035c00] to-[#024700] rounded-2xl p-6 text-white relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-xl font-extrabold mb-2 leading-snug">
          Does Your Child Want to Study at a Top Government School?
        </h3>
        <p className="text-sm text-white/85 leading-relaxed mb-4">
          JGPS offers expert coaching for AMU, JMI, Sainik School, Navodaya, and Military School entrance exams. Weekly OMR tests. Experienced teachers.
        </p>
        <ul className="text-xs text-white/80 space-y-1.5 mb-5">
          {['Free counseling', 'Previous year papers', 'Weekly Mock Tests'].map((item) => (
            <li key={item} className="flex items-center gap-2">
              <FaCheckCircle className="text-yellow-400 flex-shrink-0 text-[11px]" />
              {item}
            </li>
          ))}
        </ul>
        <div className="flex flex-col sm:flex-row gap-3">
          <Link
            href="/coaching"
            className="flex-1 text-center bg-white text-[#035c00] hover:bg-yellow-300 font-extrabold text-sm py-2.5 px-4 rounded-xl transition-all shadow-sm"
          >
            View Coaching →
          </Link>
          <a
            href="https://wa.me/919997161490?text=Hello%2C%20I%20want%20coaching%20information."
            target="_blank"
            rel="noopener noreferrer"
      ...
      <ul className="space-y-2">
        {points.map((p, i) => (
          <li key={i} className="flex items-start gap-2.5 text-sm text-amber-800">
            <FaCheckCircle className="text-amber-500 flex-shrink-0 mt-0.5 text-[13px]" />
            <span>{p}</span>
          </li>
        ))}
      </ul>
        ...
        <header className="bg-white border-b border-gray-100">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-0">
            ...
            <nav aria-label="breadcrumb" className="flex items-center gap-1.5 text-xs text-gray-400 mb-5 flex-wrap">
              <Link href="/" className="hover:text-[#035c00] transition-colors">Home</Link>
              <span>/</span>
              <Link href="/blog/" className="hover:text-[#035c00] transition-colors">Blog</Link>
              <span>/</span>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <span className="bg-green-100 text-green-800 font-bold px-3 py-1 rounded-full text-[11px] uppercase tracking-wider">
                Updated: {article.modifiedDate || article.date}
              </span>
            </div>
            ...
            <div className="border-t border-gray-200 pt-4 pb-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a
                    href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('https://jgps.in/blog/' + article.slug + '/')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <a
                    href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title)}&url=${encodeURIComponent('https://jgps.in/blog/' + article.slug + '/')}`}
                    target="_blank"
                    rel="noopener noreferrer"
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="mt-10 pt-8 border-t border-gray-200 text-center">
            <p className="text-sm text-gray-500 mb-4">
              Share this article with parents who need it.
            </p>
            <a
              href={`https://wa.me/?text=${encodeURIComponent(article.title + '\n\nhttps://jgps.in/blog/' + article.slug + '/')}`}
              target="_blank"
